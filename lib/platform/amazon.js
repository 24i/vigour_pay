...
exports._platform = {
  ...
  on: {
    init () {
      ...
      script.onload = () => {
        var DO_TEST_SDK = window.beAmazon
        if (DO_TEST_SDK) {
          ...
          script_testing.onload = () => {
            window.amzn_wa.enableApiTester(window.amzn_wa_tester)
          }
    ...
    getProducts (data) {
      ...
      var reqid = window.amzn_wa.IAP.getItemData(products)
      this._loadingCatalog[reqid] = data
    },
    buy (obj) {
      window.amzn_wa.IAP.purchaseItem(obj.id)
...
function registerObserver (pay) {
  ...
  window.amzn_wa.IAP.registerObserver({
    onSdkAvailable (resp) {
    ...
    onPurchaseResponse (response) {
      var buying
      if (response.receipt) {
        buying = amazon._buying[response.receipt.sku]
      }
      if (!buying) {
        pay.emit('error', {
        ...
        if (cb) {
          ...
          switch (response.purchaseRequestStatus) {
            ...
            default:
              err = null
              receipt.purchaseToken = response.receipt &&
