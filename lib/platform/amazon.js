...
exports._platform = {
  ...
  on: {
    init () {
      ...
      script.onerror = (err) => {
        // console.error('=========== AMAZON SCRIPT ERROR:', err)
      ...
      script.onload = () => {
        // console.error('SCRIPT ONLOAD!', pay.ready.val, pay._platform.ready.val)

        // var DO_TEST_SDK = true
        var DO_TEST_SDK = window.beAmazon
        if (DO_TEST_SDK) {
          // console.log('put dat amazon test shizle')
    ...
    buy (obj) {
      // console.error('----------- amazon buy!', obj)
      var reqid = amzn_wa.IAP.purchaseItem(obj.id)
