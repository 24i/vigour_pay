    ...
    on: {
      data: {
        condition (val, next, event) {
          // console.log('[buy] triggered!', val, this.path, this.val)
          // alert('[pay product owned listener]')
          val = this.val
          if (val) {
            // alert('[pay product owned listener] set to true')
            ...
            if (storeId) {
              // alert('[buy] platform emit buy! ' + storeId)
